...
with(player){
	if(face_up !=1){
			for(i = 0; i < deckCount; i++){
				...
				if(face_up) {
					card_sprite = macros.sprite_array[deck[i, CardNumber],deck[i, ArtNumber]];
				}
				if(deck[i, CardNumber] > 0){
		...
		for(i = 0; i < deckCount; i++){
			if(deck[i, CardNumber] > 0 && !(deck_x_2 < (deck_x - (deckCount -1 - i)*(card_width+10) + i*deck_buffer_x))){
