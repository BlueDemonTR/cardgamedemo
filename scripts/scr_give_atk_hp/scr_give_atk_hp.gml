function scr_give_atk_hp(targetController, targetTypes, targetMinLevel, targetMaxLevel, targetArchetype, givenATK, givenHP, activatedCard, ignoreTarget) {
	//player,opponent,all
	//main,momentum,all
	//int64 0-12
	//int64 1-13
	...
	for (var i=0; i < appropiate_target_count_player; i++){
		...
		with(target_select[i]){
			...
			current_function = "stat_gain";
			increaseATK = givenATK;
			increaseHP = givenHP;
