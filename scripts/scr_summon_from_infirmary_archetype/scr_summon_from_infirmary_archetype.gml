function scr_summon_from_infirmary_archetype(argument0,argument1,argument2,argument3,argument4) {
	var
	...
	scr_filter_archetype_infirmary(targetArchetype,targetTypes,targetMinLevel,targetMaxLevel,true);
	for (var i = 0; i < filtered_card_count; i++){
		...
		with(legal_targets[i]){
			infirmaryCount = global.filtered_cards[i];
			cardNum = obj_player.infirmary[infirmaryCount,0];
