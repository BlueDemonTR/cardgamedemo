function scr_attack(argument0) {
	...
	for (var i = 0; i < appropiate_target_count_opponent; i++){
		if (obj_opponent.fieldCard[global.appropiate_targets[i,1]].cardStatus[0]){
			var taunt_exists = true;
			for (var j = 0; j < appropiate_target_count_opponent; j++){
				if (!obj_opponent.fieldCard[global.appropiate_targets[j,1]].cardStatus[0]){
	...
	if((!taunt_exists || scr_check_shared(obj_player.field[attackingCard,0],0)) && !obj_player.fieldCard[attackingCard].cardStatus[12]){
		target_select[i] = instance_create_layer(x,y,"UpperInstances",obj_appropiate_targets)
		with(target_select[i]){
			player = obj_opponent
